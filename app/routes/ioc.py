...
def bulk_export():
    ...
    try:
        audit = AuditService()
        user_id = str(current_user.id) if hasattr(g, 'current_user') else 'system'
        username = current_user.username if hasattr(g, 'current_user') else 'system'
        audit.log(
            action='bulk_export',
            entity_type='ioc',
            entity_id='bulk',
...
def enrich_ioc(ioc_id):
    ...
    logger.info(f'[Enrich IOC] Request for IOC {ioc_id}, user: {current_user.id}')
...
def store_enrichment(ioc_id):
    ...
    result = {
        ...
        'enriched_by': current_user.username
    }
