...
@images_bp.route('/<image_id>', methods=['PATCH'])
@login_required
def rename_image(image_id):
    ...
    # Only allow owner or admin to rename
    if image['uploaded_by_id'] != current_user.id and not current_user.has_permission('image.rename'):
        abort(403)
    ...
    try:
        updated_image = service.rename_image(image_id, new_filename)
