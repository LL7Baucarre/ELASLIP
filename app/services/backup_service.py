        ...
        try:
            ...
            for index_name, index_data in backup_data['indices'].items():
                ...
                try:
                    ...
                    # Restore documents using Bulk API (much faster)
                    operations = []
                    for doc in docs:
                    ...
                    # Execute bulk operation if there are documents
                    if operations:
                        response = self.es_service.client.bulk(operations=operations)
                        
                        # Count successfully indexed documents
                        if response.get('errors', False):
                            # Count successful items despite errors
                            for item in response.get('items', []):
                                index_resp = item.get('index', {})
                                if index_resp.get('status') in [200, 201]:
                                    total_docs += 1
                        else:
