    ...
    def _get_case_iocs_with_relations(self, case_id: str) -> List[Dict]:
        ...
        for ioc_id in ioc_ids[:20]:  # Limit to 20
            try:
                stix_obj = self._get_stix_object(ioc_id)
                if stix_obj:
                    # Include first-level relations for this object (like the graph)
                    stix_obj['_first_level_relations'] = self._get_first_level_relations(ioc_id)
                    items.append(stix_obj)
                else:
                    # Log that the object was not found
                    logger.warning(f"STIX object {ioc_id} not found in ioc or stix_objects index")
            except Exception as e:
                logger.warning(f"Error fetching STIX object {ioc_id}: {e}")
        return items
