...
class IOCService(BaseListService):
    ...
    def _enrich_with_metadata(self, doc: Dict) -> Dict:
        ...
        if 'x_metadata' in doc and isinstance(doc['x_metadata'], dict):
            ...
            # Extract old-style fields if they exist in x_metadata
            for old_key in ['threat_level', 'tlp', 'campaigns', 'status', 'risk_score']:
                if old_key in metadata and old_key not in doc:
                    doc[old_key] = metadata[old_key]
    ...
    def update(self, ioc_id: str, updates: Dict, user_id: str = None, username: str = None) -> Optional[Dict]:
        ...
        # Get current metadata values for risk score calculation
        threat_level = updates.get('threat_level', existing.get('x_threat_level'))
        confidence_val = updates.get('confidence', existing.get('confidence'))
        # If confidence came as string, convert it for risk score calc
        if isinstance(confidence_val, str):
            confidence_val = self.CONFIDENCE_SCORES.get(confidence_val.lower(), 50)
        tlp = updates.get('tlp', existing.get('x_tlp'))
        
        # Calculate and update risk score
        update_doc['x_risk_score'] = self.calculate_risk_score(threat_level, confidence_val, tlp)
